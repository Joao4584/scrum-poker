  ...
  if (props.voters.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-slate-700 bg-slate-950/40 p-3">
        <p className="text-xs text-slate-400">{t("planning.panel.noVotes")}</p>
      </div>
    );
  }
      ...
      <div className="grid gap-2.5">
        {visibleVoters.map((voter, voterIndex) => (
          <div
            ...
            className="flex items-center justify-between gap-3 rounded-xl border border-slate-800/90 bg-slate-950/70 px-3 py-2.5"
          >
            <div className="flex min-w-0 items-center gap-3">
              <Avatar className="h-10 w-10 border border-slate-700/80 ring-1 ring-slate-800">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-slate-100">{formatDisplayName(voter.name) ?? voter.name}</p>
                <p className="truncate text-[11px] text-slate-400">{toBRFormat(voter.voted_at)}</p>
              </div>
            </div>
            <div className="flex h-10 min-w-10 shrink-0 items-center justify-center rounded-xl border border-slate-700/80 bg-slate-900/90 px-3">
              <span className="text-sm font-semibold text-slate-100">{props.revealed ? voter.value : t("planning.panel.hiddenVote")}</span>
            </div>
          </div>
        ))}
      </div>
